...
describe("The old switcheroo",() =>{

  it("Solution 1",() => {
    function vowel2index(str) {
      var result = '';
      for (let i = 0; i < str.length; i++) {
        result += /[aeiou]/i.test(str[i]) ? i+1 : str[i];
      }
      return result;
    }

    assert.equal(vowel2index('this is my string'),'th3s 6s my str15ng');
    assert.equal(vowel2index('Codewars is the best site in the world'),'C2d4w6rs 10s th15 b18st s23t25 27n th32 w35rld');
    assert.equal(vowel2index('Tomorrow is going to be raining'), 'T2m4rr7w 10s g1415ng t20 b23 r2627n29ng');
    assert.equal(vowel2index(''), '');
    assert.equal(vowel2index("90's cornhole Austin, pickled butcher yr messenger bag gastropub next level leggings listicle meditation try-hard Vice. Taxidermy gastropub gentrify, meh fap organic ennui fingerstache pickled vegan. Seitan sustainable PBR&B cornhole VHS. Jean shorts actually bitters ugh blog Intelligentsia. Artisan Kickstarter DIY, fixie cliche salvia lo-fi four loko. PBR&B Odd Future ugh fingerstache cray Wes Anderson chia typewriter iPhone bespoke four loko, Intelligentsia photo booth direct trade. Aesthetic Tumblr Portland XOXO squid, synth viral listicle skateboard four dollar toast cornhole Blue Bottle semiotics."),
